...
export function ReportsPage() {
  ...
  // Use calculateMemberScore from library to ensure consistency with Overview and Leaderboard
  const calculateMemberScore = useCallback(async (
    ...
    // Use library function - same as Overview and Leaderboard
  ...
  const fetchDetailedMetrics = useCallback(async () => {
    ...
    try {
      ...
      const squadAPromises = squadABrands.map(async (brand) => {
        ...
        const reactivationCodes = brandReactivationCustomers.map((c: any) => String(c.unique_code || '').trim()).filter(Boolean);
        const recommendCodes = brandRecommendCustomers.map((c: any) => String(c.unique_code || '').trim()).filter(Boolean);
        const extraCodes = brandExtraCustomers.map((c: any) => String(c.unique_code || '').trim()).filter(Boolean);
